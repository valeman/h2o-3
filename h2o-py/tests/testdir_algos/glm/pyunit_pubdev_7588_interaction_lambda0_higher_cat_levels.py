...
def interactions_GLM_Binomial():
    ...
    assert model0.coeff().length == model0CoeffLen, "Lambda=0, Expected coefficient length: {0}, Actual: " \
                                                    "{1}".format(model0CoeffLen, model0.coeff().length)
    assert model1.coeff().length == model1CoeffLen, "Lambda=0.001, Expected coefficient length: {0}, Actual: " \
                                                    "{1}".format(model1CoeffLen, model1.coeff().length)
